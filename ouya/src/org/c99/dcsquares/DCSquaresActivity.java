...
public class DCSquaresActivity extends Activity  implements GLSurfaceView.Renderer {
	...
	protected int GAME_HEIGHT = 450;
	
	private Random rnd;
	private double last_time_redraw = System.currentTimeMillis();
	private double last_fps_dump = System.currentTimeMillis();
    ...
    public void onDrawFrame(GL10 gl) {
        double elapsedms = System.currentTimeMillis() - last_time_redraw;
		last_time_redraw = System.currentTimeMillis();
        if(System.currentTimeMillis() - last_fps_dump > 1000) {
        	Log.i("DCSquares", "FPS: " + frames);
        	frames = 0;
        	last_fps_dump = System.currentTimeMillis();
        } else {
        	frames++;
        }
        Rect p = null;
			...
			if(player != null && p.intersects((int)(s.x - s.size), (int)(s.y - s.size), (int)(s.x + s.size), (int)(s.y + s.size))) {
				switch(s.type) {
					case TYPE_SCORE:
						...
						combo++;
						squares++;
						soundPool.play(snd_score, 1.0f, 1.0f, 0, 0, 1.0f);
						break;
					case TYPE_ENEMY:
						soundPool.play(snd_gameover, 1.0f, 1.0f, 0, 0, 1.0f);
						finish();
						break;
					case TYPE_INVINC:
						soundPool.play(snd_powerup, 1.0f, 1.0f, 0, 0, 1.0f);
						break;
					case TYPE_SLOWMO:
						soundPool.play(snd_powerup, 1.0f, 1.0f, 0, 0, 1.0f);
						break;
					...
					case TYPE_PLUS1000:
						...
						break;
					case TYPE_EVIL:
						soundPool.play(snd_powerdown, 1.0f, 1.0f, 0, 0, 1.0f);
						break;
					case TYPE_SPEED:
						soundPool.play(snd_powerdown, 1.0f, 1.0f, 0, 0, 1.0f);
						break;
