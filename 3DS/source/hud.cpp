...
void render_bar(float power, float max) {
    ...
    if(powerup_mode <MINISQUARE ||
       (powerup_mode>PLUS1000 && powerup_mode<MINUS1000)) {
        glBegin(GL_QUADS);
        glVertex3f(100,100,0.02);
        glVertex3f(100+w,100,0.02);
        glVertex3f(100+w,110,0.02);
        glVertex3f(100,110,0.02);
        glEnd();
    }
    glEnable(GL_TEXTURE_2D);
    glColor4f(1.0,1.0,1.0,center_alpha);
    if(msg != NULL)
        draw_centered(msg,115,1,8);
}

void render_effects(float gt) {
    ...
    if(gt <= 4) {
        glEnable(GL_TEXTURE_2D);
        glColor4f(1.0, 1.0, 1.0, (gt<3)?limit(gt,0,1):(4.0f-gt));
        draw_centered("Collect the green squares",60,1,8);
    }
}

void render_stat(float x, float y, char const* title, char const *value) {
    glLoadIdentity();
    glTranslatef(x, y, 0.0f);
    glBindTexture(GL_TEXTURE_2D, stat_tex);
    glColor4f(1.0,1.0,1.0,1.0);
    glBegin(GL_QUADS);
    glTexCoord2f(0,0);
    glVertex3f(0,0,0.9);
    glTexCoord2f(1,0);
    glVertex3f(145,0,0.9);
    glTexCoord2f(1,1);
    glVertex3f(145,64,0.9);
    glTexCoord2f(0,1);
    glVertex3f(0,64,0.9);
    glEnd();
    
    draw_string(title, 73 - (strlen(title) * (16 / 2.0f)), 4, 1, 16);
    draw_string(value, 73 - (strlen(value) * (24 / 2.0f)), 28, 1, 24);
    glLoadIdentity();
}
...
void render_score(float gt) {
    ...
    glEnable(GL_TEXTURE_2D);
    sprintf(tmp, "%i", score[0]);
    render_stat(10,10,"Score",tmp);
    sprintf(tmp, "%i", squares[0]);
    render_stat(165,10,"Squares",tmp);
    sprintf(tmp, "%i", combo[0]);
    render_stat(10,84,"Combo",tmp);
    render_stat(165,84,"Time",format_time(gt));
...
void render_title() {
    glEnable(GL_TEXTURE_2D);
    glLoadIdentity();
    glTranslatef(SCREEN_W / 2 - 118, SCREEN_H / 2 - 31, 0);
    ...
    glVertex3f(0,0,0.9);
    glTexCoord2f(1,0);
    glVertex3f(236,0,0.9);
    glTexCoord2f(1,1);
    glVertex3f(236,62,0.9);
    glTexCoord2f(0,1);
    glVertex3f(0,62,0.9);
    glEnd();
    glLoadIdentity();
    draw_centered("(C) 2004 Sam Steele",210,1,8);
    draw_centered("Graphics by Meg Potter & Sam Steele",220,1,8);
    draw_centered("Sound effects by Eric Revilla",230,1,8);
}
